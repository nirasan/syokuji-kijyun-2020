...
func VitaminB1(output string) {
	...
	for i := 0; i <= 13; i++ {
		...
		a := ages[i]
		list = append(list,
			newVitaminB1(data.GenderMale, a[0], a[1], data.OptionNone, cols[j:j+3]),
			newVitaminB1(data.GenderFemale, a[0], a[1], data.OptionNone, cols[j+3:j+6]),
		)
	}

	list2 := make([]data.VitaminB1, 0)
	for _, d := range list {
		...
		if d.EAR.Valid && d.RDA.Valid {
			list2 = append(list2,
				newVitaminB1WithOption(d, data.OptionEarlyPregnancy),
				newVitaminB1WithOption(d, data.OptionMidPregnancy),
				newVitaminB1WithOption(d, data.OptionLatePregnancy),
				newVitaminB1WithOption(d, data.OptionBreastfeeding),
			)
		}
	}
	...
	g.MustWrite("func VitaminB1List() []VitaminB1 {\n")
	g.MustWrite("    return []VitaminB1 {\n")
	for _, d := range list {
		s := fmt.Sprintf("%#v,\n", d)
...
func newVitaminB1(g data.Gender, from, to data.Age, o data.Option, cols []string) data.VitaminB1 {
	return data.VitaminB1{
		Gender: g,
		From:   from,
		To:     to,
		Option: o,
...
func newVitaminB1WithOption(in data.VitaminB1, o data.Option) data.VitaminB1 {
	d := data.VitaminB1{
		Gender: in.Gender,
		From:   in.From,
		To:     in.To,
